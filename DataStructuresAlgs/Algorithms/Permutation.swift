...
class Permuation<T> {
    ...
    static func test() {
        let p = Permuation<Int>()
        var array = [1,2,3,4]
        
        debugPrint("Permuation")
        var permuations = p.permutations(array: &array)
        debugPrint(permuations)
        
        debugPrint("Heap Permuation")
        permuations = p.heapsPermutations(array: &array)
        debugPrint(permuations)
    }
}
