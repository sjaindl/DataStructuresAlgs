...
class RabinKarpSubstringSearch {
    ...
    static func test() {
        debugPrint("Rabin Karp")
        
        let rabinKarp = RabinKarpSubstringSearch(base: 128)
        
        debugPrint(rabinKarp.substr(of: "xxx", in: "Substring") ?? "nil")
        debugPrint(rabinKarp.substr(of: "", in: "Substring") ?? "nil")
        debugPrint(rabinKarp.substr(of: "string", in: "str") ?? "nil")
        
        debugPrint(rabinKarp.substr(of: "Sub", in: "Substring") ?? "nil")
        debugPrint(rabinKarp.substr(of: "str", in: "Substring") ?? "nil")
        debugPrint(rabinKarp.substr(of: "a", in: "shalala") ?? "nil")
    }
}
